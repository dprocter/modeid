...
distance.moved<-function(dataset, last, time.window, epoch.length){
  ...
  if (isTRUE(last)){
    ...
    this.dataa$north.shift[1:(length(this.data[,1])-epochs.shifted)]<-this.data$northing[
      epochs.shifted+1:length(this.data[,1])]
  } else{
    this.data$east.shift<-NA
    this.data$east.shift[epochs.shifted+1:length(merged.data[,1])]<-merged.data$easting[
      1:(length(merged.data[,1])-epochs.shifted)]
    this.data$north.shift<-NA
    this.data$north.shift[epochs.shifted+1:length(merged.data[,1])]<-merged.data$northing[
      1:(length(merged.data[,1])-epochs.shifted)]
  }
